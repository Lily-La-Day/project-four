    ...
    return (
      <main className="index-main">
        ...
        <button onClick={this.minusOne}
          className="show-writing-button">
          Last Writing</button>}

          {this.state.num < this.state.writings.length &&
           <button onClick={this.addOne}
             className="show-writing-button">
             Next Writing</button>}
        </div> }

        {this.props.filteredWritings &&
          <div>
            <div className="writings-container scroll">
              {this.props.filteredWritings.map((writing, i) => (
                <Link to={`/edit-writings/${writing.id}`} key={i}>
                  <h3 > {writing.title}</h3>
                </Link>
              ))}
            </div>
            ...
            {this.props.filteredWritings.map((writing, i) => (
              <div key={i}
                className ="shown-writing">
                {(this.state.num === i) &&
              <Link to={`/edit-writings/${writing.id}`}>
                <h3
                  className="writing-title" key={i}>
                  {writing.title}</h3>
                <p
                  className="index-writing-show">
                  {writing.text}</p>
                <button
                  className="show-writing-button">
                  See More</button>
            ...
            {this.state.num > 0 &&
                <button onClick={this.minusOne}
                  className="show-writing-button">
                   Last Writing</button>}
            {this.state.num < this.props.filteredWritings.length &&
               <button onClick={this.addOne}
                 className="show-writing-button">
                 Next Writing</button>}
          </div> }
